...
| Date Functions  | Description                 | Example |
| --------------- | --------------------------- | ------- |
| DATE_PART()     | Extracts year, month or day in date and cast in number  | DATE_PART('Month', date_col) |
...
Functions below are specifically used in Window function:
...
- `ROW_NUMBER()` - Sequence is incremented by n + 1. For example, 1, 2, 3, 4, 5
- `RANK()` - For example, 1, 2, 2, 4, 5
- `DENSE_RANK()` - For example, 1, 2, 2, 3, 4
...
Can use mathematical aggregations such as `MIN()`, `MAX()`, `SUM()` and `COUNT()` in Windows function.
